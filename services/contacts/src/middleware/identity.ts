 ...
 * misconfigured (a binding bypass) or unauthenticated (a public path the
 * gateway forwarded without populating identity).
 *
 * Unlike tenants, contacts does not enforce an admin guard here: the
 * gateway already validated scope (`enforceRestScope` in the gateway auth
 * middleware), and contacts accepts `apikey | oauth | session`.
 */
