...
test("POST /mcp/execute without Mcp-Session-Id returns 400 MISSING_SESSION", async () => {
  const response = await route(
    withIdentity("http://internal/mcp/execute", {
      method: "POST",
      body: JSON.stringify({ name: "persons_create", arguments: {} }),
  ...
  const inner = JSON.parse(body.content[0]!.text) as {
    error: { code: string };
  };
...
test("POST /mcp/execute with invalid JSON body returns 400 INVALID_BODY", async () => {
  ...
  assert.equal(inner.error.code, "INVALID_BODY");
});
...
test("POST /mcp/execute with missing tool name returns 400 INVALID_BODY", async () => {
  const response = await route(
    withIdentity(
      "http://internal/mcp/execute",
      {
...
test("POST /mcp/execute without identity returns 401", async () => {
  const response = await route(
    new Request("http://internal/mcp/execute", {
      method: "POST",
      body: JSON.stringify({ name: "persons_create" }),
      headers: { "Content-Type": "application/json" },
    }),
    stubEnv,
  );
