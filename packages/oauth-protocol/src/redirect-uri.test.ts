...
describe("isLoopbackRedirectUri", () => {
  it("accepts 127.0.0.1", () =>
    assert.equal(isLoopbackRedirectUri("http://127.0.0.1/cb"), true));
  it("accepts 127.0.0.1 with port", () =>
    assert.equal(isLoopbackRedirectUri("http://127.0.0.1:8080/cb"), true));
  it("accepts [::1]", () =>
    assert.equal(isLoopbackRedirectUri("http://[::1]/cb"), true));
  it("rejects localhost", () =>
    assert.equal(isLoopbackRedirectUri("http://localhost/cb"), false));
