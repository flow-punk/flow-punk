...
test("buildToolRegistry('indie') exposes compact CRM model tools", () => {
  ...
  assert.deepEqual(registry.modelTools.map((tool) => tool.name).sort(), [
    "accounts",
    "deals",
    "persons",
    "pipelines",
  ...
  assert.equal(
    registry.modelTools.some((tool) => tool.name === "contacts"),
    false,
  );
  assert.equal(
    registry.modelTools.some((tool) => tool.name === "pipeline"),
    false,
  );
  assert.equal(
    registry.modelTools.some((tool) => tool.name === "tools_search"),
    false,
  );
});
...
test("model descriptions explain relationships between CRM records", () => {
  ...
  const byName = new Map(
    adapter.listAvailableTools().map((tool) => [tool.name, tool]),
  );
