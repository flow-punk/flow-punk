...
import { AccountsRepoError, decodeCursor, encodeCursor } from "./accounts.js";

test("cursor round-trips a {createdAt, id} payload", () => {
  const payload = { createdAt: "2026-04-28T12:00:00.000Z", id: "acct_abc" };
...
test("decodeCursor rejects malformed base64", () => {
  assert.throws(
    ...
    (err: unknown) =>
      err instanceof AccountsRepoError && err.code === "invalid_input",
  );
});

test("decodeCursor rejects non-JSON payload", () => {
  // base64url for "not-json" — decodes cleanly but JSON.parse fails.
...
test("decodeCursor rejects missing keys", () => {
  const encoded = encodeCursorRaw({ id: "acct_abc" });
  assert.throws(
    () => decodeCursor(encoded),
    (err: unknown) =>
      err instanceof AccountsRepoError && err.code === "invalid_input",
...
test("decodeCursor rejects wrong types", () => {
  const encoded = encodeCursorRaw({ createdAt: 123, id: "acct_abc" });
  assert.throws(
    () => decodeCursor(encoded),
    (err: unknown) =>
      err instanceof AccountsRepoError && err.code === "invalid_input",
