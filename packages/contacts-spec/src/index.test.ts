...
test("Account response shape includes the actual table columns (catches Phase-1 drift)", () => {
  ...
  for (const ghost of ["name", "employeeCount", "annualRevenue"]) {
    assert.equal(
      account.properties[ghost],
      undefined,
      `invented field "${ghost}" should not exist`,
    );
  }
});
...
test("AccountCreate requires displayName and excludes audit columns", () => {
  ...
  assert.ok(
    create.required?.includes("displayName"),
    "displayName must be required",
  );
  for (const audit of [
    "id",
    "createdAt",
    ...
    assert.equal(
      ...
      undefined,
      `${audit} should be excluded from POST body`,
    );
  }
});
